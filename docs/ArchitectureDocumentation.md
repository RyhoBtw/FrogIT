...
| Risk ID | Type           | Description | Probability (1–5) | Impact (1–5) | Risk Score (P x I) | Mitigation Strategy | Indicators | Contingency Plan | Status |
|--------|---------------|-------------|-------------------|--------------|-------------------|---------------------|------------|------------------|--------|
| R-01 | Technical | Real-time audio latency exceeds acceptable threshold. Simultaneous playback of multiple sounds and frog animations may cause audible delays or artifacts. | 3 | 4 | 12 | Dedicated audio thread (ADR-0001); SFML audio with hardware-accelerated OpenAL backend; limit concurrent sound sources. | Audible crackling/latency > 50 ms; frame drops below 30 FPS during audio playback. | Simplify audio mixing; limit number of concurrent sounds; increase audio thread priority. | Open |
| R-02 | Technical | Cross-platform transparency incompatibilities. Per-pixel transparency works differently on Windows (DWM), Linux (X11 Shape Extension), and macOS. SFML 3.0 provides no unified API for this. | 4 | 4 | 16 | Platform-specific implementations behind unified interface; early testing on all target platforms in CI pipeline. | CI build failures on a platform; black background on frog window in test results. | Fallback to semi-transparent background with opacity hint; alternative: render frog in main window instead of separate overlay. | Occurred & mitigated |
| R-03 | Technical | Excessive resource consumption with many frog instances. Each frog creates its own OS window with its own rendering context. At high counts (up to 200), RAM/GPU usage increases significantly. | 3 | 3 | 9 | Texture sharing via ResourceManager; frame rate limit at 60 FPS; lazy shape mask calculation only on direction change. | RAM > 500 MB; FPS < 30 with > 50 frogs; OS reports too many open windows. | Introduce soft limit; alternative: render all frogs in a single transparent fullscreen overlay. | Open |
| R-04 | Quality | Insufficient GUI test coverage. Interactive UI elements (ImGui sliders, frog click, drag & drop) cannot be tested automatically. Regressions go undetected. | 4 | 3 | 12 | Strict separation of UI and business logic; unit tests for all non-GUI components; manual test protocols for GUI. | Test coverage < 40%; recurring GUI bugs after refactoring. | Manual test checklist per release iteration; evaluate screenshot-based regression tests. | Open |
| R-05 | Organizational | Knowledge loss due to uneven task distribution. Most commits and technical knowledge are concentrated on 1–2 team members (bus factor). | 3 | 4 | 12 | Mandatory code reviews on PRs; pair programming for critical modules; knowledge transfer via blog posts (see GitHub Discussions). | Bus factor = 1 for a module; team member cannot solve assigned task without help. | ADRs for all architecture decisions; thorough documentation for complex areas (X11 integration, audio thread). | Open |
| R-06 | Technical | Dependency on third-party libraries (SFML 3.0, Dear ImGui). Breaking changes in new versions or abandoned maintenance could block further development. SFML 3.0 is relatively new and less battle-tested than 2.x. | 2 | 4 | 8 | Pin library versions in CMake (FetchContent with fixed tags); abstraction layer between app logic and framework (ResourceManager, window wrapper). | SFML/ImGui release with breaking changes; build failures after dependency update. | Fall back to last working version; if abandoned: evaluate migration to SDL2/SDL3. | Open |
| R-07 | Technical | Missing or corrupt audio files at runtime. ResourceManager terminates the app on missing files with exit(-1) instead of graceful degradation. | 3 | 3 | 9 | Defensive error handling: warning instead of crash; fallback to silence; asset validation at startup. | Crash reports with 'Failed to load' message; user complaints about missing sounds. | Embedded fallback audio; app continues without audio (graceful degradation). | Open |
| R-08 | Planning | Feature scope exceeds available capacity. SRS defines 5 features, Feature 4 (Frog Trolling) has high effort. Team size (4) and semester timeframe limit capacity. | 4 | 2 | 8 | MoSCoW prioritization: Must-Have (Soundscape, Animation), Should-Have (Interactive Frogs, Presets), Could-Have (Trolling); iterative sprints. | Velocity drops; features are postponed to next sprint. | Reduce scope to Must-Have features; document Could-Have as 'Future Work'. | Open |
